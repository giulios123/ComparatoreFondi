...
Il contratto pubblico sara' composto da `HoldingPosition`, `FundHoldings`,
`FundCoverage`, `PairOverlap`, `PortfolioExposure` e `OverlapReport`, esposto da
`analyze_overlap(funds)`. Il primo campo di ogni fondo sara' un ID opaco; il
nome visuale restera' facoltativo e non sara' necessario per i calcoli.
...
riscalate. Il valore e' mostrato anche con copertura parziale, ma sempre come
limite inferiore; con dati assenti o non validi il risultato e' `n/d`.
Per il portafoglio, ogni posizione contribuisce con `peso_fondo *
quota_posizione`; la differenza rispetto al 100% e' la parte non coperta dai
dati disponibili.

Il matching prova nell'ordine identificatore stabile con tipo, simbolo
normalizzato conservando il suffisso di borsa e nome Unicode normalizzato.
Un fallback collegato a piu' identita' forti resta ambiguo e viene segnalato
invece di fondere righe. Le quote devono essere finite e appartenere a `(0, 1]`;
duplicati risolti sulla stessa identita' vengono sommati. Un totale oltre 1 con
tolleranza viene marcato non valido.

Una data holdings piu' vecchia di 180 giorni resta utilizzabile, ma compare
come dato datato. Yahoo valorizza la fonte e lascia la data sconosciuta perche'
il suo endpoint corrente non la espone.
...
| `comparatore/sources/base.py` | Identificatore opzionale, tipo e data delle holdings, senza rendere obbligatori i nuovi campi |
...
| `app.py` | Matrice, dettaglio coppie, concentrazione aggregata, copertura e avvisi di copertura |
...
portafoglio, convenzioni `NON_CLASSIFICATO`/provenienza di `allocazione.py` e
il risultato strutturato che verra' consumato dalla spec 008.
